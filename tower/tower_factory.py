...
class Tower:
    ...
    def PCR(cls, x: int, y: int):
        ...
        :return: object
        """
        # let pcr has these imputs(coordinate_x, coordinate_y,
        #     snipe class in tower.attack_strategy, and the pcr image
        pcr = cls(x, y, Snipe(), PCR_IMAGE)
        # set the cd time by assign the cd_max_count instance attribute
        pcr.cd_max_count = 120
        # assign the range
        pcr._range = [100, 105, 110, 115, 120, 125]
        # decide the damage of pcr
        pcr._damage = [3.0, 3.2, 3.4, 3.6, 3.8, 4.0]
        # return the pcr class(I hope this is the right term)
        return pcr
    ...
    def draw_effect_range(self, win):
        ...
        transparency = 120
        pygame.draw.circle(surface, (128, 128, 128, transparency),
                           self.rect.center, self._range[self.level])
